...
class HandleShutTests(IsolatedAsyncioTestCase):
    async def test_rejects_non_admin(self):
        plugin = SimpleNamespace(admin_users=[], _shut_until_by_group={})
    ...
    async def test_rejects_private_chat(self):
        plugin = SimpleNamespace(admin_users=["1001"], _shut_until_by_group={})
    ...
    async def test_cancel_shut(self):
        plugin = SimpleNamespace(admin_users=["1001"], _shut_until_by_group={"5001": time.time() + 300})
        event = _FakeEvent(group_id="5001", is_admin=True)
    ...
    async def test_show_shut_status_when_active(self):
        plugin = SimpleNamespace(admin_users=["1001"], _shut_until_by_group={"5001": time.time() + 60})
        event = _FakeEvent(group_id="5001", is_admin=True)
...
class HandleDbTests(IsolatedAsyncioTestCase):
    async def test_rejects_non_admin(self):
        plugin = SimpleNamespace(admin_users=[], dao=SimpleNamespace(get_db_stats=AsyncMock()))
    ...
    async def test_reset_initiates_confirmation(self):
        plugin = SimpleNamespace(admin_users=["1001"], _pending_db_reset={}, dao=SimpleNamespace())
        event = _FakeEvent(is_admin=True)
        result = await admin_commands.handle_db(event, plugin, "reset")
        self.assertIn("确认", result)
        self.assertEqual(plugin._pending_db_reset["1001"]["action"], "reset")

    async def test_rebuild_initiates_confirmation(self):
        plugin = SimpleNamespace(admin_users=["1001"], _pending_db_reset={}, dao=SimpleNamespace())
        event = _FakeEvent(is_admin=True)
        result = await admin_commands.handle_db(event, plugin, "rebuild")
        self.assertIn("删除插件数据库文件并重建", result)
        self.assertEqual(plugin._pending_db_reset["1001"]["action"], "rebuild")

    async def test_confirm_times_out(self):
        plugin = SimpleNamespace(
            admin_users=["1001"],
            _pending_db_reset={"1001": {"action": "reset", "expires_at": time.time() - 1}},
            dao=SimpleNamespace(),
        )
        event = _FakeEvent(is_admin=True)
    ...
    async def test_confirm_reset_success(self):
        plugin = SimpleNamespace(
            admin_users=["1001"],
            _pending_db_reset={"1001": {"action": "reset", "expires_at": time.time() + 30}},
            dao=SimpleNamespace(
    ...
    async def test_confirm_rebuild_success(self):
        plugin = SimpleNamespace(
            admin_users=["1001"],
            _pending_db_reset={"1001": {"action": "rebuild", "expires_at": time.time() + 30}},
            dao=SimpleNamespace(
                db_path="D:/skills/GD/astrbot_plugin_self_evolution/data/self_evolution.db",
                delete_and_rebuild=AsyncMock(
                    return_value={
                        "deleted_files": ["self_evolution.db", "self_evolution.db-wal"],
                        "rebuilt": True,
                        "db_path": "D:/skills/GD/astrbot_plugin_self_evolution/data/self_evolution.db",
                    }
                ),
        ...
        event = _FakeEvent(is_admin=True)
        result = await admin_commands.handle_db(event, plugin, "confirm")
        self.assertIn("数据库文件已删除并重建", result)
        self.assertIn("self_evolution.db", result)
        self.assertNotIn("1001", plugin._pending_db_reset)
...
class HandleSetSanTests(IsolatedAsyncioTestCase):
    async def test_san_show_available_to_non_admin(self):
        plugin = SimpleNamespace(
            admin_users=[],
            san_system=SimpleNamespace(enabled=True, value=75, max_value=100, get_status=lambda: "精力充沛"),
        )
        event = _FakeEvent(is_admin=False)
    ...
    async def test_san_show_returns_disabled_when_san_off(self):
        plugin = SimpleNamespace(admin_users=[], san_system=SimpleNamespace(enabled=False))
        event = _FakeEvent(is_admin=False)
        result = await admin_commands.handle_san_show(event, plugin)
    ...
    async def test_set_san_rejects_non_admin(self):
        plugin = SimpleNamespace(admin_users=[], san_system=SimpleNamespace(enabled=True))
        event = _FakeEvent(is_admin=False)
        result = await admin_commands.handle_set_san(event, plugin)
        self.assertEqual(result, "权限拒绝：此操作仅限管理员执行。")

    async def test_show_returns_when_san_disabled(self):
        plugin = SimpleNamespace(admin_users=["1001"], san_system=SimpleNamespace(enabled=False))
        event = _FakeEvent(is_admin=True)
        result = await admin_commands.handle_set_san(event, plugin)
        self.assertEqual(result, "SAN 精力系统未启用")
    ...
    async def test_invalid_value_rejected(self):
        plugin = SimpleNamespace(
            admin_users=["1001"],
            san_system=SimpleNamespace(enabled=True, value=50, max_value=100, get_status=lambda: "略有疲惫"),
        )
        event = _FakeEvent(is_admin=True)
